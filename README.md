...
...
```
Here, we collect some more information printed by our DPORStateReducerWithSummary class. In Table 2 number 28, we will see `79` in column **Evt.** (i.e., number of events) and under the column **With DPOR**, we will see under **Trans.** the number `2,277` which is the number of unique transitions for DPOR (please keep in mind that a transition can be revisited multiple times in our algorithm because of the cyclic nature of the state space).
...
Compared to the DPOR runs, there are 2 differences for the [`moreStatistics` file](https://github.com/uci-plrg/iotcheck-dpor/blob/main/sample_logs/exampleNoDPOR/moreStatistics):
...
2. We will see statistics for 2 pairs because they finished running. However, it will show `0` for `Number of events` and `Number of unique transitions (DPOR)` because those numbers are only updated by the DPORStateReducerWithSummary class (see [this function in the class](https://github.com/uci-plrg/iotcheck-dpor/blob/feff258204f90485a4fe49cea0c38d358a228b8e/dpor_implementation/jpf-core/DPORStateReducerWithSummary.java#L186)) when we run IoTCheck with DPOR. Thus, we only take the `Number of transitions` from this file and put it under the column **Without DPOR** (under **Trans.**), e.g., `10,500` for the pair `medicine-management-temp-motion--circadian-daylight` (number 42 in Table 3).
